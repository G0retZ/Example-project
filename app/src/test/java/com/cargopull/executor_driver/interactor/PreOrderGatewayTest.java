...
public class PreOrderGatewayTest {
  ...
  public void setUp() {
    ...
    gateway = new PreOrderGatewayImpl(stompClient, mapper);
    ...
    when(stompClient.topic("/queue/1234567890", StompClient.ACK_CLIENT_INDIVIDUAL))
        .thenReturn(Flowable.just(
            new StompMessage("MESSAGE",
                Collections.singletonList(new StompHeader("Preliminary", "")), "\n")
        ));
  ...
  public void answerShiftOpenedForTotalCostIfConnected() throws Exception {
    // Дано:
    when(mapper.map(any())).thenReturn(order);
    when(stompClient.isConnected()).thenReturn(true);
    when(stompClient.topic("/queue/1234567890", StompClient.ACK_CLIENT_INDIVIDUAL))
        .thenReturn(Flowable.just(
            new StompMessage("MESSAGE",
